...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Isaiah Bennett"
date: "`r Sys.Date()`"
...
Version 2.1 \| First Created July 7, 2021 \| Updated October 5, 2023
...
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% pivot_longer(cols = covid_rate:y, names_to = "Variable", values_to = "Value") %>% kable(caption = "Missing Data") %>% kable_styling(full_width = FALSE)
...
## Map Disability Percents By Race

**Planned deviation for reanalysis:** We also map the percent of people with any disability for each racial category to investigate any spatial trends that might correspond with Covid-19 incidence clustering trends.
...
**Spatial Distribution of COVID-19 Cases and Disability** \\ The choropleth map we made in the **first part** of the reproduction analysis (Figure 2) reveals an identical spatial pattern to that of the original study's Figure 1, confirming the equivalence of our dependent variable with that of the original study.
...
The additional Percent Disability by Race Map showed an important clustering of Black people with any disabilities throughout much of the southern part of the country where there was an uneven distribution of COVID-19 cases.
These coinciding clusters points to a disproportionate impact of COVID-19 on the Black disabled community.\
...
**Correlation Analysis and Changes in Methodology** \\ The summary statistics for the **second part** of the reproduction analysis matched perfectly with those of the original study, confirming that the reproduction uses identical original data.
...
**Variations in Kulldorff Spatial Scan Statistic** \\ The Kulldorff spatial scan statistic varied significantly between the original study and our reproduction and re-analysis with regards to selection of secondary clusters, interpretation of clusters, and relative risk scores.
...
**Cluster-Based Risk Classification** \\ The original study classified COVID risk using a local relative risk score for the county at the center of each cluster (Figure 4).
...
**Challenges and Limitations in Spatial Analysis** \\ Our analysis of COVID-19 clusters in the context of controlling for spatial dependence has revealed several challenges and limitations to application of Kulldorff spatial scan statistics across different computational environments.
...
**GEE Results** \\ In the **third part** of our reproduction analysis, the results from each the five GEE models were largely consistent with Chakraborty's, suggesting robustness to modeling decisions with regards to GEE clusters.
...
**Inferential Model Comparisons** \\ We found that reproducing the analysis with **geepack** resulted in slightly different GEE results than the original study, even when using data provided by the author from the original study.
...
**Research Design and Sources of Uncertainty** \\ With regards to the **overall conclusions** and **research design**, we agree with the original author's cautious interpretation emphasizing "county-level associations" and the need for "additional data and analysis".
...
In the second reproduction analysis, we improved our approaches to reproduction studies by revising pre-analysis plans and post-analysis reports into legible code RMarkdown documents.
...
In this third analysis, efforts were focused on improving and fixing the rendering of figures and graphics in addition to the creation of new maps that helped investigate the descriptive statistics of disability percent by race and enable further comparison of these statistics with the overall COVID-19 incidence rates.
